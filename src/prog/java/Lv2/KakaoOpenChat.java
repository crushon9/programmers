...
public class KakaoOpenChat {
    public static String[] solution(String[] record) {
        ...
        for (int i = 0; i < record.length; i++) {
            ...
            if (rcd[0].equals("Enter") || rcd[0].equals("Change")) {
                // put : 이미 키값이 있을 경우 최신값으로 덮어씀
                userInfo.put(rcd[1], rcd[2]);
                ansLn++;
            }
        }
        // Enter와 Change 일때만 메세지를 출력하므로 answer길이 초기화
        ...
        for (int i = 0; i < record.length; i++) {
            ...
            if (rcd[0].equals("Enter")) {
                answer[i] = userInfo.get(rcd[1]) + "님이 들어왔습니다.";
            } else if (rcd[0].equals("Leave")) {
                answer[i] = userInfo.get(rcd[1]) + "님이 나갔습니다.";
            }
        }
        return answer;
    }
